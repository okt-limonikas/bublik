...
class Command(BaseCommand):
    ...
    def update_project_test_comments(self, project, runs_to_migrate_ids):
        ...
        for mtc in meta_test_comments:
            upd_test_comment_data = MetaTestSerializer(mtc).data
            upd_test_comment_data.pop('serial')
            upd_test_comment_data['project'] = project.id

            serializer = MetaTestSerializer(data=upd_test_comment_data)
            serializer.update_data()
            serializer.is_valid(raise_exception=True)

            serializer.get_or_create(serializer.validated_data)
        self.stdout.write(

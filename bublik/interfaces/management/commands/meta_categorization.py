...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            choices=[*Meta.projects.values_list('value', flat=True), None],
        )
    ...
    def handle(self, *args, **options):
        ...
        project = Meta.projects.get(value=project_name) if project_name else None
        project_id = project.id if project else None

        logger.info(
            f'removing all meta categories associated with the {project_name} project',
        )
        MetaCategory.objects.filter(project=project).delete()

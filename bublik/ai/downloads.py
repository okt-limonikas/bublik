...
from bublik.core import chat_storage
...
async def download_file(request: Request) -> Response:
    ...
    bounded by ``CHAT_FILE_MAX_SIZE`` enforced at generation time.
    """
    ...
    url = await anyio.to_thread.run_sync(
        chat_storage.public_download_url,
        chat_file.storage_key,
        chat_file.filename,
    )
    if url:
        return RedirectResponse(url, status_code=307)

    data = await anyio.to_thread.run_sync(chat_storage.read_object, chat_file.storage_key)

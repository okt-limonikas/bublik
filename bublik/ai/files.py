...
downloadable file, store it via :mod:`bublik.core.chat_storage` and record a
...
def _persist(deps: ChatDeps, filename: str, content_type: str, data: bytes) -> ChatFile:
    ...
    file_id = uuid.uuid4()
    key = chat_storage.chat_file_key(str(thread.pk), str(file_id), filename)
    chat_storage.upload_bytes(key, data, content_type)

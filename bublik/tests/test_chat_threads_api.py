...
class ChatThreadsApiTest(ReloadUrlconfMixin, APITestCase):
    def setUp(self):
...
class ChatDisabledUrlsTest(SimpleTestCase):
    ...
    def test_chat_thread_routes_are_registered_when_enabled(self):
        with override_settings(CHAT_ENABLED=True), reload_urlconf():

...
def set_tags_categories_cache(project_id):
    ...
    important_tags_data = tags.filter(
        ...
        category__project__id=project_id,
    ).order_by('category__priority', 'name')

    relevant_tags_data = tags.filter(
        Q(category__priority__range=(4, 9)) & Q(category__project__id=project_id)
        | Q(category__isnull=True),
    ).order_by('category__priority', 'name')

    tags_data = tags.filter(
        Q(category__priority__range=(1, 9)) & Q(category__project__id=project_id)
        | Q(category__isnull=True),
    ).order_by('category__priority', 'name')

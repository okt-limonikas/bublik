...
Selected by :mod:`bublik.core.chat_storage` whenever ``S3_ENDPOINT_URL`` is
set -- the bundled SeaweedFS service, or any S3-compatible endpoint (real AWS
S3 included). All functions are synchronous boto3 calls; async callers must
run them in a thread (``anyio.to_thread.run_sync`` / ``sync_to_async``).

...
Used when no S3 endpoint is configured (see :mod:`bublik.core.chat_storage`).
Object keys are reused verbatim as paths relative to ``CHAT_FILE_STORAGE_DIR``,
so the on-disk layout mirrors the bucket layout: ``chat/<thread>/<file>/<name>``.
...
from bublik.core.chat_storage import setting
...
def root() -> Path:
    ...
    """
    configured = setting('CHAT_FILE_STORAGE_DIR')
    if not configured:
        msg = (
            'CHAT_FILE_STORAGE_DIR is not set, but no S3 endpoint is '
            'configured either, so chat-generated files have nowhere to go. '
            'Set CHAT_FILE_STORAGE_DIR to a writable directory, or set '
            'S3_ENDPOINT_URL to use S3-compatible object storage.'
        )
